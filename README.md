...
> **⏰ 2026-07 완전 자동화(v2.1):** GitHub Actions 스케줄(`.github/workflows/daily-post.yml`, **월·수·금 09:03 KST**)에서
> [claude-code-action](https://github.com/anthropics/claude-code-action)이 `.github/daily_prompt.md` 지시서를 수행해
> 글 작성부터 발행·기록까지 **무인 운영**됩니다. 지시서에는 실측 근거 필수·일반론 금지 등 콘텐츠 품질 기준이 포함됩니다.
...
blog-automation/
├── .github/
│   ├── workflows/daily-post.yml  # 주 3회(월·수·금) 자동 발행 워크플로우
│   └── daily_prompt.md           # 자동 발행 지시서 (품질 기준 포함)
├── src/
...
```
GitHub Actions 크론(월·수·금 09:03 KST) 또는 수동 트리거
    ↓
...
| `NOTION_DASHBOARD_DB_ID` | 발행 이력을 저장할 Notion DB ID |
| `THREADS_ACCESS_TOKEN` | Threads API Access Token |
| `THREADS_USER_ID` | Threads 사용자 ID |
| `CLAUDE_CODE_OAUTH_TOKEN` | Claude Code OAuth 토큰 (GitHub Actions 자동 발행용) |

> 자동 발행은 위 값들을 **GitHub Actions Secrets**로 등록해 사용합니다 (로컬 실행은 `.env`).
> v1 레거시 스크립트 사용 시에만 `ANTHROPIC_API_KEY`, `SLACK_BOT_TOKEN`이 추가로 필요합니다.

### 4. 실행 (v2)

평소에는 **GitHub Actions가 월·수·금 09:03(KST)에 자동 실행**합니다 (Actions 탭에서 `Run workflow`로 수동 트리거 가능).
로컬 Claude Code 세션에서 "오늘 글 써서 발행해줘"라고 해도 `CLAUDE.md` 워크플로우에 따라
원고 작성 → 발행까지 수행합니다. 드래프트 수동 발행은:
