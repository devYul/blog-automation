...
- <excerpt> 태그로 글의 짧은 요약(120~160자)을 별도 출력.
  본문 도입부의 핵심을 자연스러운 한두 문장으로 정리.
  시리즈 안내가 아니라 글 본문 내용 요약이어야 함."""

_MOCK_TITLE = "Claude Code로 블로그 자동화 시작하기 - Day 1 세팅 기록"
_MOCK_EXCERPT = "직장 다니면서 AI 자동화로 부업을 만들 수 있을까? Claude API와 WordPress REST API를 연결해 블로그 글을 자동 발행하는 파이프라인을 구축한 Day 1 기록이다."
...
- 도입부 핵심을 정리한 짧은 요약을 <excerpt> 태그로 별도 출력"""
    ...
    title, content, excerpt = _parse_response(raw)
...
def _parse_response(raw: str) -> tuple[str, str, str]:
    """<title>/<content>/<excerpt> 태그에서 제목, 본문, 요약을 추출합니다."""
    import re
