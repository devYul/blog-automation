...
- <seo_title> 태그: 검색 결과에 표시될 제목. 핵심 키워드 앞에 오도록. 60자 이내.
- <meta_description> 태그: 검색 결과 설명. 자연스럽고 클릭 유도. 120~155자.
- <focus_keyword> 태그: 이 글의 핵심 SEO 키워드 한 개 (2~4단어)"""
...
_MOCK_SEO_TITLE = "Claude API + WordPress 블로그 자동화 — devYul 개발자 부업"
_MOCK_META_DESCRIPTION = "5년차 프리랜서 개발자가 Claude API와 Python으로 WordPress 블로그 자동화를 구축한 실전 기록. 설정부터 자동 발행까지 단계별로 정리했습니다."
_MOCK_FOCUS_KEYWORD = "블로그 자동화 개발자"
...
def generate_blog_post(
    ...
    if _USE_MOCK:
        return {
            ...
            "seo_title": _MOCK_SEO_TITLE,
            "meta_description": _MOCK_META_DESCRIPTION,
            "focus_keyword": _MOCK_FOCUS_KEYWORD,
            "topic": topic,
...
- <seo_title> 태그로 SEO 제목 출력 (60자 이내, 핵심 키워드 포함)
- <meta_description> 태그로 메타 설명 출력 (120~155자)
- <focus_keyword> 태그로 핵심 키워드 출력 (2~4단어)"""
    ...
    title, content, excerpt, category, seo_title, meta_description, focus_keyword = _parse_response(raw)
...
def _parse_response(raw: str) -> tuple[str, str, str, str, str, str, str]:
    """각 태그에서 title, content, excerpt, category, seo_title, meta_description, focus_keyword를 추출합니다."""
    import re
    ...
    title            = _get(r"<title>(.*?)</title>",                   "제목 없음")
    content          = _get(r"<content>(.*?)</content>",                raw)
    excerpt          = _get(r"<excerpt>(.*?)</excerpt>")
    category         = _get(r"<category>(.*?)</category>",              "개발 실전 기록")
    seo_title        = _get(r"<seo_title>(.*?)</seo_title>")
    meta_description = _get(r"<meta_description>(.*?)</meta_description>")
    focus_keyword    = _get(r"<focus_keyword>(.*?)</focus_keyword>")

    return title, content, excerpt, category, seo_title, meta_description, focus_keyword
